...
export async function POST(request: NextRequest) {
  try {
    ...
    const { configuratorId, customerEmail, customerName, customerPhone, selectedOptions, totalPrice, configuration } = body;

    const validation = validate([
      { field: 'customerEmail', value: customerEmail, rules: ['required', 'email'] },
    ...
    const response = fail(error.message, 'CREATE_ERROR', error.statusCode || 500);
    return addCorsHeaders(response, request, ['*']);
  }
}
