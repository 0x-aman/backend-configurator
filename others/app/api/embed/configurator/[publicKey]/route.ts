...
export async function GET(
  ...
  try {
    ...
    const published = configurators.find(c => c.isPublished && c.isActive);

    if (!published) {
      const response = fail('No published configurator found', 'NOT_FOUND', 404);
    ...
    const response = fail(error.message, 'EMBED_ERROR', error.statusCode || 500);
