...
export function canCreateConfigurator(status: SubscriptionStatus, currentCount: number): boolean {
  const limits = getPlanLimits(status);
  return limits.configurators === -1 || currentCount < limits.configurators;
}

export function canMakeRequest(status: SubscriptionStatus, monthlyRequests: number): boolean {
  const limits = getPlanLimits(status);
  return monthlyRequests < limits.requests;
}

export function hasFeature(status: SubscriptionStatus, feature: keyof typeof PLAN_LIMITS.FREE): boolean {
  const limits = getPlanLimits(status);
